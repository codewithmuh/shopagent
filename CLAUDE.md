...
- **Landing + docs RE-LAID-OUT to not mirror the client (2026-06-05):** client concern — recoloring wasn't enough; the structure/copy still matched the live product they were paid to build. So restructured the *expression* (ideas/features aren't ownable, specific layout+copy shouldn't match): landing hero kept **split (chat stays in the hero, per client preference)** but redesigned distinct from the client's bare version — left-aligned copy + the chat wrapped in a **browser frame** (chrome + `app.shopagent.dev/chat`) with floating "Voice + text"/"USDC checkout" accent badges and dual emerald/teal glows (NOT centered — tried centered, client rejected it); "How it works" 3-card grid→**numbered timeline (Discover/Pay/Fulfill)**; CTA generic-dark-box→**on-brand emerald panel**; docs intro plain-H1→**branded header band with quick-jump chips ("Build with ShopAgent")**. All headlines/section labels/body copy reworded. Logo now the shared `@/components/Logo` `LogoMark` (shopping-bag + chat-dots) everywhere. Tagline kept = **"Commerce through conversation"**.
- **Landing GENUINELY REBUILT from scratch (2026-06-05) — not a reskin.** Reason: line-level diff vs the original `ai-stores` showed the frontend was still **80–97% the client's code** (a derivative work; recoloring doesn't make it copyright-free — the real question is `ai-stores` ownership/contract, which is the user's to resolve; I'm not a lawyer). So: new `src/components/marketing/ChatPreview.tsx` (independently written, replaces the client's `AnimatedChat` which is now fully removed); `page.tsx` rewritten as a **server component** (original was one monolithic client component) with all-new section components + a new Capabilities section + fresh copy. Result: **landing overlap dropped ~80% → ~5%** (remaining lines are trivial tags/Tailwind fragments). Same approved emerald look + split framed-chat hero.
- **Docs: targeted rebuild DONE (2026-06-05, user picked option 2).** Rewrote the recognizable presentational components with new internals (dark terminal `CodeBlock` replacing the client's traffic-light snippet card; new `EndpointCard` palette/structure; gradient-bar `Section`; new `CopyButton`) + fresh glass nav, keeping prop signatures so the factual API content still renders. Docs overlap **~87% → ~46%**; the remaining ~46% is the functional API reference (endpoint specs, code samples) which is kept by design. Scroll-spy/IntersectionObserver logic (generic) retained.
- **Auth screens + demo chat reworked (2026-06-05):** new shared `src/components/auth/AuthShell.tsx` (full-bleed emerald backdrop + centered frosted card — distinct from the client's centered-card *and* their side-split). All 5 auth pages (demo login/signup, merchant login, portal login/register) refactored onto it; **handlers/fetch/localStorage/routing preserved byte-for-byte** (functionality unchanged). Auth overlap vs original dropped ~95% → **~25–31%** (the remainder is the kept form logic). Demo chat restyled visually only (real `LogoMark` avatars replacing the "L" letter + off-brand pink orb, `bg-app-gradient`, emoji quick-actions) — all WS/voice/state logic untouched. Also fixed lingering **Fauri "F" avatars** in `VoiceMode.tsx` + `ChatWidget.tsx` (the earlier letter-fix had missed those two components) → now `LogoMark`. Only remaining single-letter avatar is `demo/layout` "A" = the Acme storefront's own initial (intentional, separate brand). Portal/merchant sidebars already used `LogoMark`.
- **Dashboards freshened (2026-06-05):** new shared `src/components/dashboard/StatCard.tsx` (icon→value→label, `shadow-soft`+lift — distinct from the client's card). Portal + merchant dashboards render stat grids through it; `STAT_CARDS` data + all fetch/state logic untouched (StatCard takes the existing `{label,value,icon,bg,color}` shape, handles `number|string`).
- **⚖️ Still the deciding factor:** code-level similarity is now landing ~5% / docs ~46%, but ownership of the original `ai-stores` (work-for-hire/contract) is what actually settles publishability — the user's to confirm; not legal advice.
- **UI polish DONE (2026-06-05):** earlier glass/depth pass. Added shared utilities in `globals.css` (`.glass`, `.shadow-soft(-lg)`, `.card-lift`, `.bg-app-gradient`). Frosted glass on demo header + portal/merchant sidebars (over a subtle emerald/teal page gradient); landing "How it works" cards got depth (white + soft shadow + hover lift); added a **"Who it's for"** section (Shoppers / Merchants / Companies) on the landing; explanatory subtitles on the merchant + company dashboards; richer narration-friendly intro on `/docs`. All pages compile 200.
